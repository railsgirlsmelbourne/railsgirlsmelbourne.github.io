...
Now let's deploy your application to Heroku. Go to your project settings by clicking on the settings icon in the projects dropdown on the Codeship. Then navigate to the "Deployment" section. 

First you will need to pick a branch that triggers the deployment process, in this case you will probably want that to be the master branch, so select "Branch is exactly" and type in "master". Save these pipeline settings.

You will then be presented with a number of different options, as you want to deploy to Heroku, click on the "Heroku" button.

You are asked to enter the name of your Heroku application (the name as it appears on Heroku) and your API key. I sure hope you wrote that down somewhere! Enter your application's name and API key (to retrieve your Heroku API key, go to your Heroku account and click "Show API key") and save your deployment configuration.

If your deployment is failing, and you are getting a 500 Internal Server Error on Step 19 - check URL you may need to turn on "Run migrations" which can be found under the more options button in the projects deployment settings.
