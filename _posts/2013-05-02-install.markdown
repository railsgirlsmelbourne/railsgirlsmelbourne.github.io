...
Search for `powershell` in the search bar.  Right click on powershell and choose "Run as administrator"

Click Yes to allow the app to make changes to your PC

Copy the install text from the [Chocolatey](https://chocolatey.org) into the powershell window.

If you don't ge tthe right result from powershell you can try the same thing in command prompt.

Close and reopen the powershell as above.  Then type:

{% highlight sh %}
choco install cmder
{% endhighlight %}

Note: If you see an error when installing cmder, you might need to install [Visual C++ Redistributable](https://www.microsoft.com/en-au/download/details.aspx?id=48145).
...
### *2.* Update your PATH to have atom in it

Go to Control Panel

Click on System Properties -> Advanced System Settings -> Advanced -> Environment Variables.

Create a new system variable called ATOM, that will point to the folder of your  atom installation, a little bit like the screenshot below:

![ATOM variable](/images/system_property.png "ATOM variable in the system properties")

Note: if you're on Windows 10 you just add this to your PATH directly via New :)

Click OK to set the variable.

Note: if you're on Windows 10 you won't need to do this :)

Open the PATH variable and append:
