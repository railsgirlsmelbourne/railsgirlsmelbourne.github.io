...
Search for `command prompt` in the search bar.  Right click on command prompt and choose "Run as administrator"

Click Yes to allow the app to make changes to your PC

Copy the install text from the [Chocolatey](https://chocolatey.org) into the command prompt window.
...
Open `Cmder` It will be in `C:\\tools\cmder` if you can't find it through the search function
